...
let syncTimer, stateRef, loginMode = sessionStorage.getItem('vitalcare-login-mode') || (location.hash === '#portal' ? 'client' : 'admin');
sessionStorage.removeItem('vitalcare-demo-mode');
...
async function loadClientRegistration(user) {
  ...
  if (data.role === 'admin') throw new Error('ADMIN_ACCOUNT');
...
function showAuthError(error) { console.error(error); const code = error?.code || ''; const message = code === 'ADMIN_ACCOUNT' ? 'Esta cuenta es administradora. Para crear un perfil de cliente, cierra sesión e ingresa con otra cuenta de Google.' : code === 'auth/unauthorized-domain' ? 'Firebase no reconoce este dominio. Revisa los dominios autorizados.' : code === 'permission-denied' || /permission/i.test(error?.message || '') ? 'Google inició correctamente, pero Firebase rechazó el acceso a los datos. Revisa las reglas de Firestore.' : 'No fue posible completar el acceso: ' + (code || 'intenta de nuevo.'); authFeedback.textContent = message; authFeedback.classList.add('visible'); }
...
onAuthStateChanged(auth, async user => {
  ...
  try {
    ...
    if (loginMode === 'client') { if (profile.exists() && profile.data().role === 'admin') throw new Error('ADMIN_ACCOUNT'); await loadClientPortal(user); overlay.classList.add('hidden'); return; }
